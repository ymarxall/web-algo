...
export default function Struk() {
  ...
  // Load cartItems from URL query or localStorage
  useEffect(() => {
    // Try to get cartItems from URL query first
    ...
    if (cartItemsString) {
      try {
        ...
        if (Array.isArray(decodedItems) && decodedItems.length > 0) {
          ...
          setError('No valid items found in the cart. Please return to the menu to add items.');
        }
      } catch (err) {
    ...
    // Fallback to localStorage if no valid URL query
    ...
    if (storedItems) {
      try {
        ...
        if (Array.isArray(parsedItems) && parsedItems.length > 0) {
          ...
          setError('Your cart is empty. Please return to the menu to add items.');
        }
      } catch (err) {
        ...
        <div className="relative z-10">
          ...
          <div className="max-w-full mx-auto px-4 sm:px-6 py-4">
            <div className="text-center text-white">
              ...
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3">Complete Your Order</h2>
              <p className="text-white/90 text-sm sm:text-base leading-relaxed max-w-md mx-auto">
                Review your order details and provide delivery information to complete your purchase.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-full sm:max-w-4xl mx-auto">
          {error && cartItems.length === 0 ? (
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 sm:p-8 text-center w-full">
              ...
              <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">Cart is Empty</h3>
